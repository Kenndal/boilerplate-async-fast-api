# Boilerplate Asyncio FastAPI Application

A production-ready Asyncio FastAPI boilerplate with PostgreSQL, SQLAlchemy ORM, Alembic migrations, and a clean layered architecture. This project demonstrates best practices for building scalable REST APIs with proper separation of concerns, dependency injection, and comprehensive error handling.
...
- **SQLAlchemy ORM**: Asynchronous database operations with declarative models using `asyncio` and `AsyncSession`
...
- **Engine**: Asynchronous SQLAlchemy engine (`create_async_engine`) with connection pooling (`src/database/db_engine.py`)
...
- **Async Sessions**: All database operations use `AsyncSession` for non-blocking I/O

### Data Services Layer

- **Generic CRUD**: Type-safe generic CRUD operations with filtering, pagination, and sorting using async/await patterns (`src/data_services/crud.py`)
- **Filters**: Abstraction for building reusable query filters (`src/data_services/filters.py`)
- **User Data Service**: Specialized data access for user entities with async database operations (`src/data_services/user_data_service.py`)
- **Exception Handling**: Custom exceptions for unique violations and integrity errors
- **Async Operations**: All database queries use `await` with `AsyncSession.execute()` and `AsyncSession.scalars()`

### Service Layer

- **Base Service**: Generic service with async CRUD operations and result pattern error handling (`src/services/base_service.py`)
- **User Service**: Business logic for user operations with validation using async methods (`src/services/user_service.py`)
...
- **Async Methods**: All service methods are async functions that `await` data service operations

### API Layer

- **FastAPI Application**: Configured with OpenAPI docs, CORS, and global async exception handlers (`src/api_server/main.py`)
- **Routers**: RESTful async endpoints with OpenAPI documentation (`src/api_server/routers/user.py`)
- **Dependency Injection**: Async database session and service providers using `AsyncGenerator` (`src/api_server/deps.py`)
- **Problem Details**: RFC 7807-compliant error responses
- **Validation Errors**: Automatic Pydantic validation error formatting
- **Async Handlers**: All route handlers are async functions that leverage FastAPI's async support
...
FastAPI's dependency injection system is used throughout with async generators:

- `get_db()`: Async generator that provides `AsyncSession` instances with automatic transaction management
- `get_user_data_service()`: Async function that creates data service instances with injected async sessions
- `get_user_service()`: Async function that creates service instances with injected data services

This pattern enables easy testing with mocked dependencies and leverages asyncio for non-blocking I/O operations.
...
Tests use pytest with `pytest-asyncio` for async test support and mocked `AsyncSession` instances.
...
Database sessions are managed via async context managers in `get_db()`:

- `AsyncSession` context manager handles session lifecycle
- Transactions begin automatically with `async with session.begin()`
- Successful requests commit automatically
- Exceptions trigger automatic rollback
- Sessions are closed after each request
- All database operations are non-blocking using asyncio
